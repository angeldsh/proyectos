...
@WebServlet(name = "actualizaEmpleadosServlet", value = "/empleados/actualizar")
public class ActualizaEmpleadoServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try{
            ...
            empleado = empleadoDao.get(idActualizar);
        }catch (Exception ex){}
        request.setAttribute("empleado", empleado);
        empleados = empleadoDao.findAll();
        request.setAttribute("empleados", empleados);
        request.getRequestDispatcher("/empleado/actualiza_empleado.jsp").forward(request,response);
